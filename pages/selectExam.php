...
<body class="g-sidenav-show bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="container py-4">
            <h4 class="text-center mb-4">Select Exam</h4>
            <?php if (!empty($exams)): ?>
                    ...
                    <table id="assignmentsTable" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php foreach ($exams as $exam): ?>
                                <tr>
                                    <td class="text-primary"><?php echo htmlspecialchars(ucfirst($exam['exam_type'])); ?></td> <!-- Display Exam Type -->
                                    ...
                                    <td>
                                        <?php if ($exam['exam_type'] == 'theory'): ?>
                                            <!-- For Theory Exams, use a different link -->
                                            <a href="takeTheoryExam.php?id=<?php echo $exam['id']; ?>"
                                                class="btn btn-primary"
                                                <?php echo $exam['button_disabled'] ? 'disabled' : ''; ?>> <!-- Disable button if attempted -->
                                                <?php echo $exam['button_disabled'] ? 'Already Attempted' : $exam['button_text']; ?> <!-- Use button text -->
                                            </a>
                                        <?php else: ?>
                                            <!-- For Regular Exams -->
                                            <a href="takeExam.php?id=<?php echo $exam['id']; ?>"
                                                class="btn btn-primary"
                                                <?php echo $exam['button_disabled'] ? 'disabled' : ''; ?>> <!-- Disable button if attempted -->
                                                <?php echo $exam['button_disabled'] ? 'Already Attempted' : $exam['button_text']; ?> <!-- Use button text -->
                                            </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const currentPage = 'Select Exam'; // Update this dynamically based on your context
