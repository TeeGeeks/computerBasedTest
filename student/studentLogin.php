...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Basic SQL query to find the student in the database
    $query = "SELECT * FROM students WHERE email='$email_or_username' OR username = '$email_or_username'"; // Change 'teachers' to 'students'
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            $_SESSION['student_email'] = $row['email']; // Store student email in session
            $_SESSION['user_role'] = $row['role']; // Assuming 'role' exists in students table

            // Redirect to student dashboard
            showSweetAlert('success', 'Login Successful', 'Welcome to your dashboard!', '../dashboard.php'); // Change redirect to student dashboard
        } else {
