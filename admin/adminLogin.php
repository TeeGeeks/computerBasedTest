...
$conn = connection(); // Create a connection to the database
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Basic SQL query to find the admin in the database
    $adminQuery = "SELECT * FROM admins WHERE email='$email_or_username' OR username = '$email_or_username'";
    $adminResult = $conn->query($adminQuery);

    if ($adminResult->num_rows > 0) {
        $adminRow = $adminResult->fetch_assoc();

        // Check if password matches (assuming password is stored in plaintext)
        if ($password === $adminRow['password']) {
            $_SESSION['admin_id'] = $adminRow['admin_id']; // Store admin ID in session
            $_SESSION['admin_email'] = $adminRow['email']; // Store admin email in session
            $_SESSION['user_role'] = $adminRow['role']; // Store role in session

            // Redirect to admin dashboard
            showSweetAlert('success', 'Login Successful', 'Welcome to your admin dashboard!', '../dashboard.php');
        } else {
            showSweetAlert('error', 'Login Failed', 'Invalid admin password. Please try again.');
        }
    } else {
